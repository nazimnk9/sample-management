...
export default function AddSpacePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to the current parent space if we have parentUid, otherwise go to root
      router.push(parentUid ? `/space?currentParentUid=${parentUid}` : "/space")
    } catch (err) {
      toast({ title: "Error", description: "Failed to create space", variant: "destructive" })
    } finally {
      setIsLoading(false)
  ...
  const getBackUrl = () => {
    return parentUid ? `/space?currentParentUid=${parentUid}` : "/space"
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full px-4 py-2.5 border rounded-lg bg-card text-foreground placeholder:text-muted-foreground/50/50 focus:outline-none focus:ring-2 focus:border-transparent transition ${fieldErrors.description ? "border-red-500 focus:ring-red-500" : "border-border focus:ring-primary"
                    }`}
                  rows={4}
                />
