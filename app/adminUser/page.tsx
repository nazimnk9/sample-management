...
export default function AdminUserPage() {
  ...
  useEffect(() => {
    const checkAccessAndFetchUsers = async () => {
      ...
      // Check if user can access admin user list
      if (!canAccessFeature(role, "admin_user_list")) {
        toast({
          title: "Access Denied",
          description: "You do not have permission to access admin users.",
          variant: "destructive",
        })
      ...
      // Fetch users
      try {
        const response = await apiCall("/organizations/users/admin/user")
        if (response.ok) {
  ...
  const handleDelete = async (userId: number) => {
    if (confirm("Are you sure you want to delete this user?")) {
      try {
        const response = await apiCall(`/organizations/users/admin/user/${userId}/`, {
          method: "DELETE",
        })
        if (response.ok) {
    ...
    <div className="p-6 bg-background">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {canAccessFeature(userRole, "create_user") && (
            <Link href="/adminUser/add">
