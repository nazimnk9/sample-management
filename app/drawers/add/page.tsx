...
export default function AddDrawerPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            submitFormData.append("type", "DRAWER")
            if (image) {
            ...
            toast({ title: "Success", description: "Drawer created successfully" })
            toast({ title: "Success", description: "Drawer created successfully" })
            if (parentUid) {
                router.push(`/drawers?parent_uid=${parentUid}`)
            } else {
                router.push("/drawers")
            }
        } catch (err) {
        ...
        <div className="p-4 sm:p-6 lg:p-8 bg-background min-h-screen w-full overflow-y-auto">
            <div className="max-w-2xl mx-auto">
                <div className="mb-6 sm:mb-8 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Create New Drawer</h1>
                        <p className="text-sm sm:text-base text-muted-foreground mt-2">Add a new storage drawer</p>
                    </div>
                </div>

                <Card className="border-border">
                    <CardHeader className="pb-3 sm:pb-4">
                        <CardTitle className="text-base sm:text-lg">Drawer Details</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {/* Drawer Name */}
                            <div>
                                <label className="block text-sm font-medium text-foreground mb-2">Drawer Name *</label>
                                <input
                                    type="text"
                                    name="name"
                                    value={formData.name}
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 pt-6">
                                ...
                                <Link href="/drawers" className="flex-1">
